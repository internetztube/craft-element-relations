...
class MarkupService
{
     ...
     * @return string
     */
    public static function getMarkupFromElementRelations(string $elementRelations, int $elementId, int $siteId, bool $isElementDetail): string
    {
        ...
        $seomaticLocalElements = collect($relationsSeomaticLocal)
            ->map(function ($row) {
        ...
        $renderElementPreview = fn (...$args) => Cp::elementPreviewHtml(...$args);
        return Craft::$app->getView()->renderTemplate(
            'element-relations/_components/fields/relations-content',
            [
                'siteId' => $siteId,
                'elementId' => $elementId,
                'isElementDetail' => $isElementDetail,
                'seomaticLocalElements' => $seomaticLocalElements->all(),
                'currentSiteElements' => $currentSiteElements->all(),
                'isInSeomaticGlobal' => collect($rows)->contains(SeomaticService::IDENTIFIER_SEOMATIC_GLOBAL),
